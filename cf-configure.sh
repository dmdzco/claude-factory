...
#
# Usage:
#   ./cf-configure.sh                    # Interactive prompts
#   ./cf-configure.sh /path/to/project   # Configure for specific project
#   ./cf-configure.sh --reset            # Reconfigure from scratch
#===============================================================================
...
main() {
    ...
    if [[ -z "$target_path" ]]; then
        ...
        echo "  Enter the path to the git repository you want droids to work on."
    ...
    echo "  Worktrees will be created as: ${default_name}-1, ${default_name}-2, ..."
    echo ""
    ...
    #---------------------------------------------------------------------------
    # Step 3: Number of droids
    #---------------------------------------------------------------------------
    echo ""
    echo -e "${BLUE}How many parallel droids?${NC} (1-10)"
    read -p "Number of droids [3]: " num_droids
    num_droids="${num_droids:-3}"

    if [[ ! "$num_droids" =~ ^[0-9]+$ ]] || [[ "$num_droids" -lt 1 ]] || [[ "$num_droids" -gt 10 ]]; then
        log_error "Droid count must be between 1 and 10"
        exit 1
    fi

    log_success "Droids: $num_droids"
...
# Generated by cf-configure.sh on $(date +"%Y-%m-%d %H:%M:%S")
#===============================================================================

# Project name (used for worktree directories and branch names)
# Worktrees: ${project_name}-1/, ${project_name}-2/, ...
# Branches:  feat/droid-1-workspace, feat/droid-2-workspace, ...
...
EOF
    ...
    for i in $(seq 1 "$num_droids"); do
        echo "    ${worktree_parent}/${project_name}-${i}/"
    done
    echo ""
    echo -e "${GREEN}Run ./cf-start.sh to set up worktrees and launch droids!${NC}"
    echo ""
}
